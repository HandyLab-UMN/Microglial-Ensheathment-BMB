...
restoredefaultpath;
folder = fileparts(which('Fig2abcd_analyzeDiRT.m')); 
addpath(genpath(folder));
rmpath(folder)

%% Load the averaged data
load('./DiRT_Sim_Data/DiRT_Results_sym_N50_tau0.10_ave.mat')

%%
alphaFn = @(b,t) b(1)/b(2)^2*t.*exp(-t/b(2));
% First term estimated for phi = -1 and then fixed at 55.2924
modelFn3 = @(b,t) 55.2924*exp(-t/b(1));

% Only go up to phi = 0.9 for the symmetric case
phi_vec = phi_vec(1:find(phi_vec==0.9));
